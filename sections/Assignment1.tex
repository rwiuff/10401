...
\begin{wrapfigure}[14]{r}{.4\textwidth}
	...
	\label{ShTh}
\end{wrapfigure}
Given \cref{eq:inner_ellipse} and \cref{eq:outer_ellipse}, the engineering volume can easily be derived if $c\,\cos(\phi)$ and $\kappa\, c\,\sin(\phi)$ is added to the x and y-direction in \cref{eq:outer_ellipse} respectively, where $c$ is the minimum thickness of the magnetic coils that provide the torroidal field. \\
